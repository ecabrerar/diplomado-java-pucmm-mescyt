...
// Model for Pais entity
    models.Pais = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/jsp-twitter-bootstrap/rest/paises/",
        idAttribute: 'id',
        defaults: {
            descripcion: ""
        },
    ...
    // Collection class for Pais entities
    models.PaisCollection = Backbone.Collection.extend({
        model: models.Pais,
        url: "http://localhost:8080/jsp-twitter-bootstrap/rest/paises",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
// Model for org.diplomado.pucmm.mescyt.java.petcareapp.javaee.entidades.Pais entity
    models.Pais1 = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/jsp-twitter-bootstrap/rest/paises/static/",
        idAttribute: 'id',
        defaults: {
            descripcion: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'update') {
                options.url = 'http://localhost:8080/jsp-twitter-bootstrap/rest/paises/' + model.id;
            }
            if (method == 'delete') {
                options.url = 'http://localhost:8080/jsp-twitter-bootstrap/rest/paises/' + model.id;
            }
    ...
    // Collection class for org.diplomado.pucmm.mescyt.java.petcareapp.javaee.entidades.Pais entities
    models.Pais1Collection = Backbone.Collection.extend({
        model: models.Pais1,
        url: "http://localhost:8080/jsp-twitter-bootstrap/rest/paises/static",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PaisCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-pais-list-itemis template identifier for item
                        templateName: '#tpl-pais-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.pais = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.pais,
                    // tpl-pais-details is template identifier for chosen model element
                    templateName: '#tpl-pais-details',
    ...
    /*var AppRouter1 = Backbone.Router.extend({
     ...
     list:function () {
     this.collection = new models.Pais1Collection();
     var self = this;
     this.collection.fetch({
     success:function () {
     self.listView = new views.ListView({
     model:self.collection,
     // tpl-pais1-list-itemis template identifier for item
     templateName : '#tpl-pais1-list-item'
     });
     $('#datatable').html(self.listView.render().el).append(_.template($('#thead').html())());
     if (self.requestedId) {
     self.details(self.requestedId);
     ...
     details:function (id){
     if (this.collection) {
     this.pais1= this.collection.get(id);
     if (this.view) {
     this.view.close();
     }
     var self = this;
     this.view = new views.ModelView({
     model:this.pais1,
     // tpl-pais1-details is template identifier for chosen model element
     templateName: '#tpl-pais1-details',
     getHashObject: function(){
     return self.getData();
     }
     });
