...
// Model for Raza entity
    models.Raza = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/jsp-twitter-bootstrap/rest/razas/",
        idAttribute: 'id',
        defaults: {
            nombre: ""
        },
    ...
    // Collection class for Raza entities
    models.RazaCollection = Backbone.Collection.extend({
        model: models.Raza,
        url: "http://localhost:8080/jsp-twitter-bootstrap/rest/razas/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RazaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-raza-list-itemis template identifier for item
                        templateName: '#tpl-raza-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.raza = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.raza,
                    // tpl-raza-details is template identifier for chosen model element
                    templateName: '#tpl-raza-details',
